...
##############################
# Get proxies & user-agents  #
##############################

function get_ip_useragent() {
        a=`od -vAn -N4 -tu4 < /dev/urandom | sed 's/ //g'`
        i=0;

# Storm proxies residential IP : Port from 5001 to 5050
        # port=5001;
        # while [ ${port} -le 5050 ]
        # do
        ...
        PROXY_ARR[$i]="172.16.2.14:3528"; let "i=i+1";
        PROXY_ARR[$i]="172.16.2.14:3128"; let "i=i+1";
        # Storm proxies in 15 minutes: Port from 6001 to 6040
